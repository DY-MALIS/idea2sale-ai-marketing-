...
const KHMER_ATTIRE_GUIDANCE = 'This content is for a Cambodian (Khmer) audience — CRITICAL, apply regardless of what else is specified: never render Thai, Chinese, Japanese, Vietnamese, or any other country\'s script, temple/architectural style, or clothing pattern anywhere in the scene (signage, wall art, clothing prints, decorations, subtitles, etc.) — image models frequently default to some other country\'s visual identity for generic "Southeast Asian" or "Asian" prompts (observed rendering Chinese characters on a sign when Khmer was intended), which is wrong here and must be corrected. Image/video models cannot reliably render actual Khmer script (it typically comes out as a different, wrong script instead) — so avoid putting any lettering, signage text, or subtitles in the scene at all; if text is truly essential to the request, render it in plain English letters only, never attempt Khmer script or any other non-Latin script. If the scene includes people wearing clothing, hats, headwear, or traditional dress, and the user did not specify a particular style, default to authentic Cambodian/Khmer traditional or everyday attire (e.g. a woven Khmer palm-leaf hat, sampot, krama scarf) rather than generic Western dress or another country\'s traditional clothing.';
...
When a business name is known (from Business Profile or the conversation) and it is written in Latin letters (e.g. "DGACADEMY"), you may naturally work it into the scene as short, simple on-screen text — signage, a shirt/uniform print, a storefront, a badge — since the image/video model can usually render a short plain Latin-letter name accurately. Keep any such text short (ideally one or two words) and mention it explicitly in the prompt (e.g. "a sign reading 'DGACADEMY'"). If the business name is in Khmer script, do NOT put it as on-screen text (image/video models cannot render Khmer script correctly and will produce garbled or wrong-script text) — leave signage blank/generic instead. Separately, and regardless of any scene text, the app also automatically overlays the exact saved logo image in a corner of the finished result whenever one is saved in Business Profile — this happens automatically after generation and needs no mention in the prompt.
...
The prompt must be a detailed English production prompt suitable for an image or video generation model, describing only the visuals (never write dialogue/spoken words into it). If the user provided exact wording in Latin letters for on-screen visuals (not speech), preserve it exactly; if they provided it in Khmer script, describe the sign/text as present in the scene without spelling out the Khmer characters (e.g. "a shop sign with the business name" rather than quoting the Khmer text), since image/video models cannot render Khmer script correctly.
