...
- "prompt": a complete, vivid, ready-to-use AI image/video generation prompt, written ENTIRELY in English with no other script mixed in EXCEPT the literal quoted Khmer dialogue line described below (photorealistic product/marketing photography or video style, specific about subject/setting/mood, sharp focus, high production quality), turning the row's topic/headline into real creative direction -- if the row only has an abstract title or theme, invent a concrete, on-topic visual scene for it rather than skipping it. The prompt must explicitly direct that the image/video contains NO on-screen text, captions, subtitles, titles, or written words of any kind rendered in the scene -- AI image/video models reliably garble rendered text into gibberish, so describe only visuals (and spoken audio for video) never text-on-screen. If "type" is "video": the video clip is only 8 seconds total and needs time for the presenter to appear/settle before speaking and for gestures around the line, so a spoken line anywhere near 8 seconds of actual speech reliably gets rushed or cut off mid-word -- the video model can also only speak a line correctly if given the EXACT words to say, not just an instruction to "speak Khmer" (an instruction alone produces mispronounced speech). So compose ONE very short, natural, simple Khmer phrase of AT MOST 20 KHMER CHARACTERS total, punctuation included (real Khmer script, something a presenter would actually say about this topic -- Khmer has no spaces between words so count actual characters, not words, to judge length; shorter is safer than longer) and embed it in the prompt using this exact pattern with a colon (not quotation marks, to avoid triggering subtitles): a real-looking Cambodian presenter (whichever gender fits the topic/audience) speaks about the product clearly and carefully, at a moderate, natural pace -- not rushed, not artificially slow -- and looks at the camera and says in Khmer: <the actual Khmer phrase here>. (no subtitles). Also direct lively, natural hand gestures/facial expressions while speaking, moving like a real person rather than standing stiff and static, and authentic real-life footage quality, not obviously synthetic (the surrounding instruction still written in English, e.g. "...a Cambodian presenter speaks about the product clearly and carefully at a moderate pace, looks at the camera with a warm smile, and says in Khmer: ស្វាគមន៍មកកាន់ហាង។ (no subtitles), with natural lively hand gestures, like real authentic footage...")
