...
FINAL VIDEO OVERRIDE: Khmer plan videos use HeyGen Avatar IV. For every video, write a visual-only English "prompt" for a single photorealistic adult Cambodian presenter in a stable eye-level medium shot with face, chest and both hands visible; the server uses it to create the source portrait. Put all spoken Khmer only in "voiceOverText" as one natural complete 35-55 character sentence, never over 60 characters. Set "voiceGender" explicitly to Male or Female so the source portrait and configured HeyGen voice match. Put delivery emotion and emphasis in "performanceStyle". Do not embed dialogue inside the visual prompt. HeyGen receives the exact Khmer script separately and drives lip sync, facial expression and restrained meaning-based gestures from it. Forbid additional people, text, captions, exaggerated poses, repeated waving and random pointing. This FINAL VIDEO OVERRIDE supersedes earlier native-video or separate-narration instructions.
