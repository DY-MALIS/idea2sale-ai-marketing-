...
- "prompt": a complete, vivid, ready-to-use AI image/video generation prompt, written ENTIRELY in English with no other script mixed in (photorealistic product/marketing photography or video style, specific about subject/setting/mood, sharp focus, high production quality), turning the row's topic/headline into real creative direction -- if the row only has an abstract title or theme, invent a concrete, on-topic visual scene for it rather than skipping it. The prompt must explicitly direct that the image/video contains NO on-screen text, captions, subtitles, titles, or written words of any kind rendered in the scene -- AI image/video models reliably garble rendered text into gibberish, so describe only visuals (and for video, spoken audio) never text-on-screen. If "type" is "video", the prompt must explicitly direct all of the following: the video is SILENT -- no spoken dialogue, no narration, no voice-over, and no on-screen text/subtitles of any kind (AI video models cannot yet produce clear, correctly-pronounced Khmer speech or lip-sync -- every attempt has come out mispronounced, cut off mid-word, or out of sync with the mouth, so spoken narration must not be requested at all); instead direct a real-looking Cambodian presenter/actor with lively, natural, expressive hand gestures, facial expressions, and body language (smiling, nodding, gesturing at the product) that communicates the topic visually without needing speech, moving like a real person rather than standing stiff and static; and the video should look like authentic real-life footage, not obviously synthetic or robotic (the instruction itself still written in English, e.g. "...a Cambodian presenter with warm, natural, lively expressions and hand gestures, silent (no speech, no dialogue, no on-screen text), like real authentic footage...")
