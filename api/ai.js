...
- "prompt": a complete, vivid, ready-to-use AI image/video generation prompt, written ENTIRELY in English with no other script mixed in (photorealistic product/marketing photography or video style, specific about subject/setting/mood, sharp focus, high production quality), turning the row's topic/headline into real creative direction -- if the row only has an abstract title or theme, invent a concrete, on-topic visual scene for it rather than skipping it. The prompt must explicitly direct that the image/video contains NO on-screen text, captions, subtitles, titles, or written words of any kind rendered in the scene -- AI image/video models reliably garble rendered text into gibberish, so describe only visuals (and for video, spoken audio) never text-on-screen. If "type" is "video", the prompt must explicitly direct all of the following: a real-looking Cambodian presenter/narrator (pick whichever gender genuinely fits the row's topic/audience -- do not default to always the same one) speaks on camera SLOWLY and clearly in simple, natural Khmer, enunciating each word deliberately and pausing briefly between phrases so the speech is easy to understand (never on-screen text/subtitles for it); keep the spoken line itself short (under 12 words) so the model has room to pronounce it clearly rather than rushing; they have lively, natural, expressive hand gestures and facial expressions while speaking, moving like a real person on camera rather than standing stiff and static; and the video should look like authentic real-life footage, not obviously synthetic or robotic (the instruction itself still written in English, e.g. "...a Cambodian presenter speaks slowly and clearly in simple Khmer, enunciating each word, saying a short line, with natural lively hand gestures and expressions, like real authentic footage, no on-screen text...")
