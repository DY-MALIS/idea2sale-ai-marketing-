...
- The app can automatically hand a complete visual brief to its existing Image Generator or Video Creator. When the user explicitly wants an image or video, collect only the missing essentials and do not ask for details that can be safely inferred.
...
const buildCreativeAutomation = async ({ message, historyText, responseLanguage }) => {
  ...
  const rawPlan = await generateOpenRouterText({
...
Prefer asking one clarifying question (ready=false) before generating, unless the request is already fully specific (clear subject, style, and any brand/logo handling) or the user explicitly says to generate/create it now.
If the user mentions a logo or brand mark, do not plan for the image model to draw that logo itself (AI image models render specific logos and text inaccurately). Instead set ready=false and ask whether they want the app's saved business logo automatically overlaid on the finished image instead — the app already does this automatically when a logo is saved in Business Profile, so the scene itself should not depict the brand's logo.
