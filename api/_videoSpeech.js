...
export async function preparePlanVideoSpeech(item) {
  ...
  return {
    ...
    motionPrompt: `LIP-SYNC AND REAL-TIME MOTION HAVE HIGHEST PRIORITY. ${performanceStyle} Keep exactly one audible primary speaker in a continuous stable-camera shot; supporting people may perform subtle context-appropriate activity but remain silent and never lip-sync. The primary face stays nearly frontal and fully visible; limit head turns to a few degrees and keep hands and objects below the chin. Match each lip closure, jaw opening and facial motion directly to the supplied audio with no delayed mouth start, extra mouthing or continued speech after the audio ends. Use natural blinking and two crisp, purposeful hand or task gestures tied to the two spoken clauses. Each primary gesture starts promptly with its phrase, completes in about half a second, and returns to rest without lingering. All body and hand movement plays at normal real-life speed with confident energy; supporting actions also move naturally rather than in slow motion. No slow camera push, slow motion, long pose, repeated waving, nodding loop, random pointing, oversized gesture, dancing, theatrical acting or distracting background motion.`,
  };
}
